...
module.exports = {
    restDeleteRows: function (params, callback) {
        ...
        async.autoInject({
            ...
            "queryString": function (queryParam, callback) {
                var queryString = require(global.ROOT_DIR + "/common/unit/baseRest/queryString");
    ...
    restInsertRows: function (params, callback) {
       var dal = new da.dataAccess();
        async.autoInject({
            ...
            "queryString": function (queryParam, columntype, callback) {
                var queryString = require(global.ROOT_DIR + "/common/unit/baseRest/queryString");
                queryString.insertQueryString(params, queryParam, columntype, callback);
            },
            "columns": function (queryString, callback) {
    ...
    restSonGetRows: function (params, callback) {
       var dal = new da.dataAccess();
        async.autoInject({
            ...
            "queryString": function (queryParam, columntype, callback) {//合成sql语句
                var queryString = require(global.ROOT_DIR + "/common/unit/baseRest/queryString");
                queryString.getSonQueryString(params, queryParam, columntype, callback);
            },
    ...
    restUuid:function (params,callback) {
        var uuidNumber=params.urlparam.number;
        var outUuid=new Array();
      
        var uuidGet= require(global.ROOT_DIR + '/common/unit/uuidGet');
        for(var i=0;i<uuidNumber;i++)
        {
            outUuid[i]=uuidGet.getUuid();
        }
        callback(null,outUuid);
    },
    restSchGetRows:function (params,callback) {
        var dal = new da.dataAccess();
        async.autoInject({
            "open": function (callback) {
                dal.open(false, callback);
            ...
            "tableField":function (queryParam,callback) {
                getTableField(dal,"SELECT * FROM CIGPROXY.TABLEFIELD WHERE TABLENAME= " + "'" + queryParam[0] + "'",callback);
            },
            "bootstracpTablefields":function (queryParam,callback) {
                getTableField(dal,"SELECT A .*, B. NAME,B. ALIAS FROM CIGPROXY.BSTABLEFIELD A LEFT JOIN CIGPROXY.TABLEFIELD B ON	A .FIELDID = B.FIELDID WHERE (A.ORDERABLE=1 OR A.SORTABLE=1 OR SEARCHABLE=1) AND B.TABLENAME= " + "'" + queryParam[0] + "'",callback);
            },
            "formfields":function (queryParam,callback) {
                getTableField(dal,"SELECT A .*, B. NAME,B. ALIAS FROM CIGPROXY.FORMFIELD A LEFT JOIN CIGPROXY.TABLEFIELD B ON	A .FIELDID = B.FIELDID WHERE (A.READONLY=1) AND B.TABLENAME=" + "'" + queryParam[0] + "'",callback);
            },
        }, function (err, res) {
            dal.close(function () {var queryString = require(global.ROOT_DIR + "/common/unit/baseRest/queryString");
            var fin=queryString.getSchemasJson(res);
            callback(err, ["get", fin]); });
    ...
    restGetRowsNumber: function (params, callback) {
       var dal = new da.dataAccess();
        async.autoInject({
            ...
            "queryString": function (searchField, queryParam, columntype, callback) {//合成sql语句
                var queryString = require(global.ROOT_DIR + "/common/unit/baseRest/queryString");
                queryString.getQueryString(searchField, params, queryParam, columntype, callback);
            },
            "count": function (queryString, callback) {
                getCount(dal, queryString, callback);
            },
    ...
    restUserRows: function (params, callback) {
         var dal = new da.dataAccess();
        async.autoInject({
            "open": function (callback) {
                dal.open(false, callback);
            ...
            "queryString": function (searchField,queryParam, columntype, user,callback) {//合成sql语句
                //user="'"+user+"'"
                params.receiveUrl.id.push(user[0]);
                params.receiveUrl.idName.push("RECEIVERID");
                params.receiveUrl.idLength++;
                params.receiveUrl.idNameLength++;
                params.receiveUrl.urlDataLength=params.receiveUrl.urlDataLength+2;
                var queryString = require(global.ROOT_DIR + "/common/unit/baseRest/queryString");
                queryString.getQueryString(searchField,params, queryParam, columntype, callback);
            },
            "columns": function (columntype, queryString, callback) {
    ...
    restUserRowsNum:function (params, callback) {
         var dal = new da.dataAccess();
        async.autoInject({
            "open": function (callback) {
                dal.open(false, callback);
    ...
    restCreateTable: function (params, callback) {
       var dal = new da.dataAccess();
        async.autoInject({
            "open": function (callback) {
                dal.open(false, callback);
            ...
            "queryString": function (callback) {
                var queryString = require(global.ROOT_DIR + "/common/unit/baseRest/queryString");
                queryString.createTableString(params, callback);//增加三条querystring
            },
            "createTable": function (queryString, callback) {
                modifyRows(dal, queryString, "create table failure", callback);
            },
            "insertFields": function (queryString, callback) {
                modifyRows(dal, queryString, "insert rows into table tablefield failure", callback);
            },
            "insertForms": function (queryString, callback) {
                modifyRows(dal, queryString, "insert rows into table formfield failure ", callback);
            },
            "insertBSTable": function (queryString, callback) {
                modifyRows(dal, queryString, "insert rows into table BSTABLEFIELD failure", callback);
            },
            "insertSCHtable": function (queryString, callback) {
                modifyRows(dal, queryString, "configure table failure", callback);
            }
        }, function (err, res) {
            if (err) {
                dal.rollback();
            ...
            // dal.close(function () { });
            // callback(err, ["insert", res.columns]);
        });
    },
