...
module.exports = {
    getQueryString: function (searchField, params, queryParam, columntype, callback) {
    ...
    insertQueryString: function (params, queryParam, columntype, callback) {
        ...
        for (var i = 0; i < alterLength; i++) {
            for (var j = 0; j < alterWidth; j++) {
                dataColumn[j][i] = alterData[inIndex];
                inIndex = inIndex + 1;
            }
        }
        var idName = queryParam[1];
        ...
        for (var i = 0; i < alterLength; i++) {
            for (var j = 0; j < alterWidth; j++) {
                userModSql_Params[j + 1] = dataColumn[j][i];
            ...
            userModSql_Params[0] = "'" + insertId[i] + "'";
            tempQuery1[i] == '';
            tempQuery1[i] = " (";
            for (var k = 0; k < alterWidth + 1; k++) {
                if (k != alterWidth)
                    tempQuery1[i] += userModSql_Params[k] + ","
                if (k == alterWidth)
                    tempQuery1[i] += userModSql_Params[k] + ",(select sysdate from dual)) ";
            }
        }
        for (var i = 0; i < alterLength; i++)
        { tempQuery2[i] = "INTO " + tableName + " (" + idName + "," + insertColumn + " VALUES " + tempQuery1[i]; }
        var tempQuery3 = '';
        for (var i = 0; i < alterLength; i++) {
    ...
    updateQueryString: function (params, queryParam, columntype, callback) {
        ...
        alterName[alterWidth] = "UPDATEDATE";
        var inIndex = 0;
        for (var i = 0; i < alterLength; i++) {
            for (var j = 0; j < alterWidth; j++) {
                dataColumn[j][i] = alterData[inIndex];
        ...
        for (var i = 0; i < alterLength; i++) {
            dataColumn[alterWidth][i] = "(select sysdate from dual)";
        }
    ...
    getSonQueryString: function (params, queryParam, columntype, callback) {
    ...
    getSchemasJson: function (res) {
        var Schemas = {};
        var tableInfo = {
            Table: res.queryParam[0],
            PrimaryKey: res.queryParam[1],
            SpecialKey: res.queryParam[3]
        };

        var tempFields = new Array();
        for (var i = 0; i < res.tableField.length; i++) {
            tempFields[i] = {};
        }
        for (var i = 0; i < res.tableField.length; i++) {
            for (var key in res.tableField[i]) {
                if (key != "ID" && key != "TABLENAME" && key != "FIELDID" && key != "CREATOR" && key != "CREATEDATE" && key != "UPDATEDATE")
                    tempFields[i][key] = res.tableField[i][key];
            }
        }
        var BootstracpTableField = new Array();
        for (var i = 0; i < res.bootstracpTablefields.length; i++) {
            BootstracpTableField[i] = {};
        }
        for (var i = 0; i < res.bootstracpTablefields.length; i++) {
            for (var key in res.bootstracpTablefields[i]) {
                if (key != "ID" && key != "TABLENAME" && key != "FIELDID" && key != "CREATOR" && key != "CREATEDATE" && key != "UPDATEDATE")
                    BootstracpTableField[i][key] = res.bootstracpTablefields[i][key];
            }
        }
        var FormField = new Array();
        for (var i = 0; i < res.formfields.length; i++) {
            FormField[i] = {};
        }
        for (var i = 0; i < res.formfields.length; i++) {
            for (var key in res.formfields[i]) {
                if (key != "ID" && key != "TABLENAME" && key != "FIELDID" && key != "CREATOR" && key != "CREATEDATE" && key != "UPDATEDATE")
                    FormField[i][key] = res.formfields[i][key];
            }
        }
        var Fields = {};
        for (var i = 0; i < tempFields.length; i++)
            Fields[tempFields[i].NAME] = tempFields[i];
        tableInfo["Fields"] = Fields;
        tableInfo["BootstrapTableFields"] = BootstracpTableField;
        tableInfo["FormFields"] = FormField;
        Schemas[res.queryParam[0]] = tableInfo;
        return Schemas;
    },
    createTableString: function (params, callback) {
        var fieldName = params[0];
        var fieldType = params[1];
        var fieldLen = params[2];
        var tableName = params[3];
        var priKey = params[4];
        var specialkey = params[5];
        var nullable = params[6];
        var defaulta = params[7];
        var queryString = "";
        ...
        for (var i = 0; i < fieldName.length; i++) {
            tempQuery1 += fieldName[i] + " " + fieldType[i] + "(" + fieldLen[i] + ") default " + defaulta[i] + " " + nullable[i] + ", ";
        }
        tempQuery1 += "primary key (" + priKey + ") )";
        queryString += tempQuery1;
        return queryString;
    },
